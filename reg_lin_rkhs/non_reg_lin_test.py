...
# cercle_points = helper.random_circle(1, 100)
...
# K_predict = sklearn.metrics.pairwise.polynomial_kernel(np.matrix(cercle_points['x']).T, Y=x_predict, degree=2)
...
x_limit = 10
dataset = helper.random_function(lambda x: np.sin(x) / x, number_points = 100, min = -x_limit, max = x_limit)
# x_test = np.matrix(helper.random_points(100, -x_limit, x_limit)).T
dataset_test = helper.random_function(lambda x: np.sin(x) / x, number_points = 100, min = -x_limit, max = x_limit)
kernelMatrix = lambda x, Y=None: sklearn.metrics.pairwise.rbf_kernel(x, Y = Y, gamma=1)

ridge_rkhs(dataset['x'], dataset['y'], dataset_test['x'], dataset_test['y'], kernelMatrix, ridge_coeff = 1)
