...
def random_circle(number_points, noise = True, rayon = 1):
    ...
    x = rayon * np.cos(random_alphas) + noise_x
    y = rayon * np.sin(random_alphas) + noise_y
...
def random_square(number_points, noise = True, variance = 1):
    return random_function(lambda x: x*x, number_points, variance)

def kernel_reg(kernel_matrix, y_train, ridge_coeff):
    # kernel_matrix should be a numpy.matrix
    n = np.shape(kernel_matrix)[0]
    inv = np.linalg.inv(kernel_matrix + ridge_coeff * np.identity(n))
    return inv.dot(y_train)
